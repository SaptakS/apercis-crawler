...
class FileManager:
	...
	def save_file(self, handler, page, crawl_time, link_score, page_score, num_terms, total_terms):
		...
		if (self.file_count >= self.cnt_limit):
			self.lock.release()
			return
		#print "Match terms%f. %s" % (float(num_terms) / float(total_terms), url)
		if (float(num_terms) / float(total_terms) < 0.5):
			self.lock.release()
			return
